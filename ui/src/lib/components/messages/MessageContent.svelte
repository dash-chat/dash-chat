<script lang="ts">
	...
	const isEdited = $derived(message.editHistory.length > 0);
	const quietClass = $derived(
		message.author === myDeviceId ? 'dark-quiet' : 'quiet',
	);
...
{/if}
{#if hasText || (metadata && !isPhotoOnly && !isFileOnly) || (isEdited && !isPhotoOnly && !isFileOnly)}
