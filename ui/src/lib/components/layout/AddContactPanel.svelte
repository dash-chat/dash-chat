<script lang="ts">
	...
	async function createMyDeepLink(): Promise<string> {
		try {
			...
			if (link === null) {
				throw new Error(`toDeepLink returned null for code ${code}`);
			}
			return link;
		} catch (e) {
		...
		{#if tab === 'code'}
			...
			{:then [deepLink, name]}
				{#await $qrColor then savedColor}
					{@render myCodeView(deepLink, name, savedColor ?? defaultQrColor())}
				{:catch}
					{@render myCodeView(deepLink, name, defaultQrColor())}
				{/await}
			{:catch error}
				<ErrorPlaceholder
...
{#snippet myCodeView(deepLink: string, name: string, color: string)}
			...
			{#if import.meta.env.DEV}
				<div class="column w-full gap-2">
					...
					<Button
						...
						onClick={() => void submitPastedCode()}>{m.addContact()}</Button
					>
				</div>
			{/if}
		</div>
	</div>
	<QrCodeUploader bind:this={uploaderRef} onSelectImage={receiveDeepLink} />
{/snippet}
