<script lang="ts">
	...
	import { Button, Link, Navbar, Preloader } from 'konsta/svelte';
	import { fileToAvatar } from '$lib/utils/image';
	import { capturePhoto, pickMedia } from '$lib/utils/media';
	...
	async function setAvatar(file: File | null | undefined) {
		if (!file) return;
		...
		>
			{#if isMobile}
				<div class="column" style="align-items: center; gap: 8px;">
					<Button
						tonal
						onClick={async () => setAvatar(await capturePhoto())}
						class="icon-only"
			...
			<div class="column" style="align-items: center; gap: 8px;">
				<Button
					tonal
					onClick={async () =>
						setAvatar((await pickMedia('image', false))?.[0])}
					class="icon-only"
