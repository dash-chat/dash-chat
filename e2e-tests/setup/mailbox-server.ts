 ...
 *  The hub binary bakes the run's network id in, like the app does. The
 *  mailbox's testing endpoints (see mailbox-control.ts) only exist behind
 *  its `test_utils` feature, so the e2e build turns it on. */
export function buildCargoPackages(packages: string[]): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		proc.on('exit', code => {
			if (code === 0) resolve();
			else reject(new Error(`cargo ${args.join(' ')} exited ${code}`));
		});
	});
}
