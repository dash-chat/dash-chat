...

{
  perSystem =
    { inputs', pkgs, ... }:
    {
      ...
      # phones on 149-152, the emulator image on 124), each from its own
      ...
      packages.e2e-chromedrivers = pkgs.linkFarm "e2e-chromedrivers" [
        {
          name = "chromedriver-152";
          path = "${inputs'.nixpkgs-chromedriver-152.legacyPackages.chromedriver}/bin/chromedriver";
        }
        {
          name = "chromedriver-151";
          path = "${inputs'.nixpkgs-chromedriver-151.legacyPackages.chromedriver}/bin/chromedriver";
        }
        {
