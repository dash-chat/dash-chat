...
- `perform` opens the chat it acts in through `openChat`, which checks the chat list's unread badges and then the chat itself against the model **on the way in**, acts, and records its ops on the acting agent. A move leaves the app wherever it finished, so one that needs the chat list asks for it (`backToChatList`) and records where it ended up (`openedChat` / `wentHome`).
...
- `expectChatList(model, sa)` reads every row of the chat list in one `execute()` and waits until each chat the model knows carries the unread badge it says — the messages that landed there while the app was elsewhere, and no badge at all on one the agent has caught up on. `openChat` runs it after `backToChatList` and before clicking the row, so it fires on nearly every move for free. Rows belonging to no chat the model holds (a contact request nobody has accepted yet, which the app badges with a 1 of its own) are left alone.
- `checkChatList(model, sa)` is the pair of them: check the chat the agent is sitting in, walk to the list, check the badges. `openChat` starts with it, and `stopApp` / `restart` run it before killing the app — a badge that reads right is a read the app has written, so nothing the model credits the agent with is still pending when the process goes (the read marking is debounced, and a kill runs no teardown).
- `openChat` checks the chat the agent is **sitting in** before it walks away from it. The model hands an agent everything a move produced at once and counts what lands in the chat on screen as read; the real ops arrive when they arrive, and one still on the wire when the app leaves would land on a chat it is no longer showing, where the row counts it unread. Waiting for the view first is what makes "the chat it is in is a chat it has read" true of both. `expectView` leaves the list scrolled to the bottom for the same reason — a message landing above the fold is never marked read.
