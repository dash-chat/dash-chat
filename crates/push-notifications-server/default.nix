...

{
  perSystem = { inputs', pkgs, self', lib, system, ... }:
    let
      ...
      rust = craneLibPkgs.rust-bin.fromRustupToolchainFile
        "${self}/rust-toolchain.toml";
      ...
      commonArgs = {
        ...
        cargoExtraArgs = "-p push-notifications-server";
      };

      cargoArtifacts = craneLib.buildDepsOnly commonArgs;

    in {
      packages.push-notifications-server =
