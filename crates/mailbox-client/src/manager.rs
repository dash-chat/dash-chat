...
mod tests {
    ...
    impl LogStore<Msg> for TrackingClient {
    ...
    #[async_trait::async_trait]
    impl RemoteBlobStore for TrackingClient {
        async fn fetch_blob(&self, _hash: OpaqHash) -> Result<Option<Opaq>, anyhow::Error> {
            Ok(None)
        }

        async fn publish_blob(&self, _blob: Opaq) -> Result<(), anyhow::Error> {
            Ok(())
        }
    }
    ...
    #[async_trait::async_trait]
    impl LogStore<Msg> for FailingClient {
        async fn publish(&self, _ops: Vec<Msg>) -> Result<(), anyhow::Error> {
            Ok(())
        }
        async fn fetch(
    ...
    #[async_trait::async_trait]
    impl RemoteBlobStore for FailingClient {
        async fn fetch_blob(&self, _hash: OpaqHash) -> Result<Option<Opaq>, anyhow::Error> {
            Ok(None)
        }
