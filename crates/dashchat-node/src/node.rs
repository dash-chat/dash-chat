...
impl Node {
    ...
    pub async fn init(
        ...
        // p2panda is spawned with no networking layer at all. The node still
        // reaches peers, through mailbox servers over HTTP. `no_p2p` alone is not
        // enough: mailbox media exchange dials the mailbox over iroh. The iOS push
        // extension turns both off, and has to, because a second endpoint under
        // the app's key takes over the app's relay session and kills the running
        // app's networking actors.
        ...
        // The builder refuses any network setting on a node it spawns without a
        // networking layer, so they are skipped rather than set to something inert.
        if no_networking {
