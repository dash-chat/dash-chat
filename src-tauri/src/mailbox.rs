...
async fn hand_over_our_addr(
    ...
    loop {
        ...
        for hub in current.values() {
            ...
            if handed_over.get(&hub.mailbox_id) == Some(&addr) {
                continue;
            }
            exchange_addrs(&node, &hub.mailbox_id, addr).await;
            handed_over.insert(hub.mailbox_id.clone(), addr);
        }
