...
- **Default the test window to wide-screen (the configured 850×650).** Pages like the direct-chat back link only render `{#if !isWideScreen.value}`, so navigation paths differ between modes. When a spec needs narrow mode, call `agent.setWideScreen(false)` in `before()`. Otherwise rely on the sidebar (`ChatListPanel` / `SettingsPanel` / `NewMessagePanel`) staying mounted across chat routes — that's why most "go back to home" steps reduce to clicking a sidebar link rather than navigating to `/`.
